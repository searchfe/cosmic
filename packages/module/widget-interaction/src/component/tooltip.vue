...
const mHDirection = ref(props.hDirection);
watchEffect(() => {
    ...
    if (props.hDirection === 'center' || props.hDirection === 'right') {
        nextTick(() => {
            getStyle();
        });
    }
...
function computedStyle() {
    ...
    // 中间临界、右侧临界
    if (screenWidth.value - iconRect.left - iconRect.width / 2 < (tooltipRect.width / 2 + 2 * props.rowTileGap)) {
